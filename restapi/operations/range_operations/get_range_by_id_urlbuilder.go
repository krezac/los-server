...
func (o *GetRangeByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/range/{rangeId}"

	rangeID := swag.FormatInt64(o.RangeID)
	if rangeID != "" {
		_path = strings.Replace(_path, "{rangeId}", rangeID, -1)
	} else {
		return nil, errors.New("RangeID is required on GetRangeByIDURL")
	}
...
func (o *GetRangeByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetRangeByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetRangeByIDURL")
	}
